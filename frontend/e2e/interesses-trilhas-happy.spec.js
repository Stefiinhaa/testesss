...
test.describe('Interesses e Trilhas - fluxo feliz desktop', () => {
  test('cria, abre edicao direta em card e remove interesse e trilha', async ({ page }) => {
    await page.addInitScript(() => {
      window.confirm = () => true;
    });

    const interesseNome = uniqueLabel('E2E Interesse');
    const trilhaNome = uniqueLabel('E2E Trilha');
    ...
    await expect(page.getByRole('button', { name: `Editar interesse ${interesseNome}` })).toHaveCount(0);
    ...
    await trilhaPane.getByRole('textbox').nth(1).fill('Trilha criada no fluxo feliz desktop');
    await trilhaPane.locator('input[type="number"]').first().fill('3');
    ...
    await expect(page.getByRole('button', { name: `Editar trilha ${trilhaNome}` })).toHaveCount(0);
  });
});
